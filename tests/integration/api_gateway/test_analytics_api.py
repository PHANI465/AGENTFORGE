...
async def test_usage_summary_aggregates_cost_records(client, api_key, session):
    ...
    session.add_all([
        CostRecordORM(id=uuid.uuid4(), agent_id=agent_id, model="gpt-4o-mini",
                       tokens_in=100, tokens_out=50, cost_usd=0.01),
        CostRecordORM(id=uuid.uuid4(), agent_id=agent_id, model="gpt-4o-mini",
                       tokens_in=200, tokens_out=80, cost_usd=0.02),
    ])
...
async def test_cost_breakdown_scoped_to_agent(client, api_key, session):
    ...
    session.add_all([
        CostRecordORM(id=uuid.uuid4(), agent_id=uuid.UUID(agent_a["id"]), model="gpt-4o-mini",
                       tokens_in=100, tokens_out=50, cost_usd=0.01),
        CostRecordORM(id=uuid.uuid4(), agent_id=uuid.UUID(agent_b["id"]), model="gpt-4o-mini",
